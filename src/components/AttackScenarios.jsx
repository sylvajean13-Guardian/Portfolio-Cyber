...
const content = {
  fr: {
    ...
    methodology:
      "Méthodologie : sur les 8 scénarios comparés avant/après déploiement des agents, 6 ont été correctement détectés — soit un taux de détection de 75 %.",
    scenarios: [
      ...
      { technique: "Brute Force WinRM", id: "T1110.001", description: "Force brute sur le service WinRM via CrackMapExec avec wordlist rockyou.txt.", detection: "Détecté", detail: "14 728 événements bruts générés en 30 minutes, corrélés par 2 règles de détection distinctes (60122, 60204) de niveau 10 — le SIEM réduit le bruit à des alertes exploitables.", status: "detected" },
      ...
      { technique: "Kerberoasting", id: "T1558.003", description: "Tentative d'extraction de ticket TGS via Impacket-GetUserSPNs pour cracking hors ligne.", detection: "Bloqué en amont", detail: "Windows Server 2025 impose AES-256 — RC4 refusé (KDC_ERR_ETYPE_NOSUPP). Bloqué par la configuration système, pas par une détection Wazuh.", status: "blocked" },
    ],
  },
  en: {
    ...
    methodology:
      "Methodology: out of the 8 scenarios compared before/after agent deployment, 6 were correctly detected — a 75% detection rate.",
    scenarios: [
      ...
      { technique: "WinRM Brute Force", id: "T1110.001", description: "Brute force on the WinRM service via CrackMapExec using the rockyou.txt wordlist.", detection: "Detected", detail: "14,728 raw events generated in 30 minutes, correlated into actionable alerts by 2 distinct level-10 detection rules (60122, 60204) — the SIEM reduces noise into workable signal.", status: "detected" },
      ...
      { technique: "Kerberoasting", id: "T1558.003", description: "TGS ticket extraction attempt via Impacket-GetUserSPNs for offline cracking.", detection: "Blocked upstream", detail: "Windows Server 2025 enforces AES-256 — RC4 rejected (KDC_ERR_ETYPE_NOSUPP). Blocked by system configuration, not by Wazuh detection.", status: "blocked" },
    ],
  },
};
  ...
  return (
    <section className="border-t border-[var(--border-color)] px-6 py-28">
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mx-auto mb-14 max-w-2xl rounded-xl border border-[var(--border-color)] bg-[var(--surface)] p-4 text-center text-xs text-[var(--muted)]">
          {t.methodology}
        </div>
          ...
          {t.scenarios.map((s, i) => {
            ...
            return (
              <motion.div
                ...
                className="rounded-2xl border border-[var(--border-color)] bg-[var(--surface)] p-6 shadow-sm"
              >
