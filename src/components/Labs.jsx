...
const labSetup = [
  {
    icon: Terminal,
    machine: "VM Windows",
    tools: ["EZTools", "Splunk"],
  },
  {
    icon: HardDrive,
    machine: "VM Kali Linux",
    tools: ["Volatility3", "Wireshark"],
  },
];
...
export default function Labs() {
  return (
    <section id="labs" className="border-t border-white/10 bg-slate-900/40 px-6 py-28">
      <div className="mx-auto max-w-7xl">
        <div className="mb-14">
          <p className="text-sm font-semibold uppercase tracking-[0.3em] text-blue-400">
            Formation pratique
          </p>
          <h2 className="mt-4 text-4xl font-bold sm:text-5xl">Labs & environnements</h2>
        </div>

        {/* Lab 1 : SOC/SIEM du mémoire */}
        <div className="mb-8 rounded-2xl border border-white/10 bg-slate-950/60 p-7">
            ...
            <table className="w-full text-left text-sm">
              ...
              <tbody className="text-slate-300">
                {socLabMachines.map((m) => (
                  <tr key={m.vm} className="border-b border-white/5">
                    <td className="py-2 pr-4 font-semibold text-blue-400">{m.vm}</td>
                    <td className="py-2 pr-4">{m.role}</td>
                    <td className="py-2 pr-4 text-slate-400">{m.os}</td>
                    <td className="py-2 font-mono text-xs text-slate-400">{m.ip}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          {labSetup.map((setup, index) => {
            ...
            return (
              ...
              >
                ...
                <h3 className="text-xl font-semibold">{setup.machine}</h3>
        ...
        <div className="mt-8 rounded-2xl border border-white/10 bg-slate-950/60 p-7">
          <div className="mb-4 flex items-center gap-2 text-blue-400">
            <Search size={20} />
            <h3 className="text-lg font-semibold text-white">Cas pratiques traités</h3>
          </div>
          <ul className="grid gap-3 text-sm text-slate-400 sm:grid-cols-2 lg:grid-cols-3">
            {caseFiles.map((c) => (
